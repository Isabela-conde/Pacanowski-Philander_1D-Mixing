...
using Printf
using PyPlot
using PyCall   # for matplotlib.animation (PyCall ships with PyPlot)
...
# ---------------------------------------------------------------------------
# Shear-instability diagnostics (all live at cell faces, like in
# pp_shear_driven.jl): gradient Richardson number Ri = N^2 / (dU/dz)^2 and the
# Ri-dependent eddy viscosity ν=KU / diffusivity κ=KT. Ri < Ri_crit (~0.25)
# is the model's signature of shear instability / active mixing.
# ---------------------------------------------------------------------------
Ri_crit = 0.25
z_mld   = 70.0    # depth band [0, -z_mld] we call the "mixed layer / surface layer"
...
# here is the shear-instability signal we want to show for the shoaled EUC.
...
function run_pp(h_core; N = 128, H = 300.0, mixing = true,
                ...
                snapshots = false, save_every = 3)   # snapshots: store U(z,t) for animation
    ...
    # minimum Richardson number in the surface layer, so we can see when/whether
    # the shoaled jet becomes shear-unstable (min Ri < Ri_crit).
    nsteps = Int(round(tfinal / Δt))
    usurf  = Float64[]; t = Float64[]; minRi = Float64[]
    tsnaps  = Float64[]; Usnaps = Vector{Float64}[]    # U(z,t) movie frames
    ...
    function grab_snap()
        ...
        push!(Usnaps, [model.solution.U[i] for i in 1:N])
    ...
    return model, t, usurf, minRi, tsnaps, Usnaps, zf, Risnaps, νsnaps
end
...
m_d, t, us_d, minRi_d, ts_d, Us_d, zf, Ris_d, νs_d = run_pp(h_deep;  snapshots = true)
m_s, _, us_s, minRi_s, ts_s, Us_s, _,  Ris_s, νs_s = run_pp(h_shoal; snapshots = true)
...
# ===========================================================================
# FIGURE 2 -- time-depth Hovmöller diagrams of Ri and ν (deep vs shoaled)
#   This captures the EARLY, TRANSIENT shear instability: a low-Ri / high-ν
#   band that lights up at the start of the run (lost in the end-of-run
#   snapshots). Red contour marks Ri = Ri_crit.
# ===========================================================================
